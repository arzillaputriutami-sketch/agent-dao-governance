/**
 * DAO Governance Radar — dApp Upgrade Wiring
...
(function() {
  ...
  async function apiFetch(path, opts = {}) {
    try {
      ...
      console.error('[DAO Governance Radar] API error:', e);
  ...
  window.dappExportJSON = function(data, filename) {
    ...
    a.href = url;
    a.download = filename || '06-dao-governance-export.json';
    a.click();
  ...
  window.dappExportCSV = function(rows, filename) {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = filename || '06-dao-governance-export.csv';
    a.click();
    URL.revokeObjectURL(url);
